		...
		<navigator url="/pages/address/address?source=1" class="address-section">
			<view class="order-content">
				...
				<view class="cen">
					...
					<text class="address"
						>{{ addressData.address }} {{ addressData.addressName }} {{ addressData.houseNo }}</text
					>
				</view>
...
export default {
	...
	onShow() {
		console.log('[option:] ', this.addressData);
	...
	methods: {
		...
		async getAddress() {
			...
			if (receivers.length > 0) {
				const defaultAddress = receivers.find((item) => item.isDefault);
				if (defaultAddress) {
					this.addressData = defaultAddress;
				} else {
					this.addressData = receivers[0];
				}
			}
		},
		// 获取优惠券列表
